  ...
  function ctrlFn(
    ...
    $scope.editInstallment = function (installment, dataField) {
      if (dataField === 'IsPaid' || dataField === 'IsAdminBan') {
        ...
        bootbox.confirm({
          ...
          callback: function (confirmResult) {
            if (confirmResult) {
              ...
              ClientsService.editVehicleRegistrationInstallment(
                context.ClientID,
                context.VehicleID,
                context.VehicleRegistrationID,
                installment.ID,
                editObj
              ).then(
                function () {
        ...
        openDateFieldDialog(dataField, installment[dataField]).then(
          function (result) {
            var editObj = {};
            editObj[dataField] = UtilityService.convertDateToISODateString(
              result.Value
            );
    ...
    $scope.resetInstallments = function () {
      bootbox.confirm({
        ...
        callback: function (confirmResult) {
          if (confirmResult) {
            ClientsService.resetVehicleRegistrationInstallments(
              context.ClientID,
              context.VehicleID,
              context.VehicleRegistrationID
            ).then(
        ...
        resolve: {
          settings: function () {
            return {
              ...
              DateValue: _.isDate(date)
                ? date
                : UtilityService.convertISODateStringToDate(date)
            };
          }
        }
      };
    ...
    function refreshInstallmentsList() {
      ClientsService.getVehicleRegistrationInstallments(
        context.ClientID,
        context.VehicleID,
        context.VehicleRegistrationID
      )
        ...
        .catch(error => {
          toastr.error(
            'Došlo je do greške prilikom osvežavanja spiska rata.\nZatvorite dijalog, pa pokušajte ponovo.'
          );
        });
    }
  }
